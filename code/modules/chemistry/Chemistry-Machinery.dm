...
/obj/machinery/chem_master
	...
	var/obj/beaker = null
	var/obj/item/reagent_containers/glass/buffer_tank = null
	var/obj/selected_container = null
	var/list/container_cache = null
	...
	New()
		..()
		src.buffer_tank = new /obj/item/reagent_containers/glass/beaker/extractor_tank(src)
		src.buffer_tank.name = "Buffer Tank"
	...
	proc/tryInsert(obj/item/reagent_containers/glass/B, var/mob/user)
		...
		if(!src.selected_container)
			src.selected_container = src.beaker
	...
	proc/eject_beaker(mob/user)
		...
		if(!src.roboworking)
			...
			if (src.beaker == src.selected_container)
				src.selected_container = null
			src.beaker = null
			src.roboworking = null
			rebuild_container_cache()
			src.UpdateIcon()
	...
	proc/check_patch_whitelist()
		if(!src.selected_container?.reagents)
			return FALSE
		if(src.emagged)
			return TRUE
		...
		for (var/reagent_id in src.selected_container.reagents.reagent_list)
	...
	proc/rebuild_container_cache()
		src.container_cache = list()
		var/containers = get_containers()
		for(var/container in containers)
			var/obj/item/reagent_containers/glass/thisContainer = containers[container]
			container_cache[container] = ui_describe_reagents(thisContainer)
			if(container_cache[container])
	...
	proc/manufacture_name(var/param_name)
		...
		if(isnull(name) || !length(name) || name == " ")
			name = null
			if(src.selected_container)
				name = src.selected_container.reagents.get_master_reagent_name()
		return name
	...
	ui_data(mob/user)
		...
		if(!QDELETED(src.selected_container))
			.["default_name"] = src.selected_container.reagents.get_master_reagent_name()
		else
			.["default_name"] = null
		if (src.rebuild_cache)
			src.rebuild_container_cache()

		var containers = get_containers()
		for(var/container in containers)
			if(container_cache[container])
				container_cache[container]["selected"] = src.selected_container == containers[container]
		.["containers"] = container_cache
	...
	ui_act(action, list/params, datum/tgui/ui)
		...
		var/list/containers = get_containers()
		var/obj/target = containers[params["container_id"]]
		switch(action)
			if("insert")
			...
			if("analyze")
				...
				if(!target?.reagents)
					return
				var/datum/reagent/reagent = target.reagents.get_reagent(id)
				if(reagent)
			...
			if("flush")
				var/id = params["reagent_id"]
				if(target?.reagents)
					var/reagent_amount = target.reagents.get_reagent_amount(id)
					target.reagents.remove_reagent(id, reagent_amount)
					if(target == src.beaker && !target.reagents.total_volume) // qol eject when empty
						eject_beaker(ui.user)
					rebuild_container_cache()
					. = TRUE
			if("flushinput")
				...
				if (target?.reagents)
					...
					. = TRUE
			if("chemtransfer")
				var/obj/from = containers[params["from_container_id"]]
				if(from?.reagents.total_volume && target?.reagents && from != target)
					from.reagents.trans_to(target, clamp(params["amount"], 1, 500))
					rebuild_container_cache()
					. = TRUE
			if("select")
				if(target)
					src.selected_container = target
					. = TRUE
			// Operations
			if("makepill")
				if(!src.selected_container || !src.selected_container.reagents.total_volume)
					return
				...
				// sanity check
				var/reagent_amount = clamp(round(params["amount"]), CHEMMASTER_MINIMUM_REAGENT, src.selected_container.reagents.maximum_volume)
				var/pill_icon = params["icon"] // handled in design_pill

				var/obj/item/reagent_containers/pill/P = new(src)
				P.name = "[item_name] pill"
				src.selected_container.reagents.trans_to(P, reagent_amount)
				...
				if(src.selected_container == src.beaker && !src.selected_container.reagents.total_volume) // qol eject when empty
					eject_beaker(ui.user)
			...
			if("makepills")
				...
				var/pillcount = round(src.selected_container.reagents.total_volume / reagent_amount)
				if(!pillcount)
					// invalid input
					boutput(ui.user, "[src] makes a weird grinding noise. That can't be good.")
					return

				logTheThing(LOG_COMBAT, usr, "used [src] to create [pillcount] [item_name] pills containing [log_reagents(src.selected_container)] at [log_loc(src)].")
			...
			if("makebottle")
				...
				src.selected_container.reagents.trans_to(bottle, reagent_amount)
			...
			if("makepatch")
				...
				src.selected_container.reagents.trans_to(patch, reagent_amount)
			...
			if("makepatches")
				...
				var/patchcount = round(src.selected_container.reagents.total_volume / reagent_amount)
				if(!patchcount)
					// invalid input
					boutput(ui.user, "[src] makes a weird grinding noise. That can't be good.")
					return
				...
				if(use_box || patchcount > CHEMMASTER_NO_CONTAINER_MAX)
					...
					if (is_medical_patch)
						patch_box.build_overlay(average = src.selected_container.reagents.get_average_color())
					else // dangerrr
				...
				logTheThing(LOG_COMBAT, usr, "used the [src.name] to create [patchcount] [item_name] patches from [log_reagents(src.selected_container)] at [log_loc(src)].")
